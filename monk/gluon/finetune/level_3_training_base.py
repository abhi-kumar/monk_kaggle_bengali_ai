...
class finetune_training(finetune_model):
    ...
    def set_training_evaluation(self):
        ...
        #kaggle competition addition
        if(self.system_dict["kaggle"]["dataset"]["percent_data"]):
            max_iterations = int(num_batch*self.system_dict["kaggle"]["dataset"]["percent_data"]/100.0);
            self.custom_print("Max iterations per epoch - {}".format(max_iterations));
        else:
            max_iterations = num_batch;
            self.custom_print("Max iterations per epoch - {}".format(max_iterations));
        ...
        for i, batch in enumerate(self.system_dict["local"]["data_loaders"]["val"]):
            ...
            if(i > max_iterations):
                self.custom_print("Validation for the epoch done on {} % of data as specified".format(self.system_dict["kaggle"]["dataset"]["percent_data"]))
                break;
        return metric.get(), test_loss, max_iterations;
    ###############################################################################################################################################
    ...
    def set_training_final(self):
        if(self.system_dict["states"]["resume_train"]):
            ...
            #kaggle competition addition
            if(self.system_dict["kaggle"]["dataset"]["percent_data"]):
                max_iterations = int(num_batch_train*self.system_dict["kaggle"]["dataset"]["percent_data"]/100.0);
                self.custom_print("Max iterations per epoch - {}".format(max_iterations));
            else:
                max_iterations = num_batch_train;
                self.custom_print("Max iterations per epoch - {}".format(max_iterations));
            ...
            for epoch in range(self.system_dict["hyper-parameters"]["num_epochs"]):
                ...
                for i, batch in enumerate(self.system_dict["local"]["data_loaders"]["train"]):
                    ...
                    if(i > max_iterations):
                        self.custom_print("Training for the epoch done on {} % of data as specified".format(self.system_dict["kaggle"]["dataset"]["percent_data"]))
                        break;

                _, train_acc = metric.get()
                train_loss /= max_iterations;

                val_acc, val_loss, max_val_iterations = self.set_training_evaluation();
                val_acc = val_acc[1];
                val_loss /= max_val_iterations;
