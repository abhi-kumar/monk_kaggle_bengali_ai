...
class prototype_params(finetune_state):
    ...
    # Extra Modules - Kaggle_Dataset_Params
    ###############################################################################################################################################
    def Kaggle_Dataset_Params(self, competition="bengaliai-cv19", data_root="/kaggle/input/bengaliai-cv19/", split=0.9, input_size=(137, 236), 
        batch_size=4, shuffle_data=True, num_processors=3, target="grapheme_root", read=[0, 1, 2, 3], percent_data=100): 
        #target = "grapheme_root", "vowel_diacritic", "consonant_diacritic", "multiclass";
        self.system_dict["kaggle"]["status"] = True;
        if(self.system_dict["states"]["eval_infer"]):
            x=0;

        else:
            self.system_dict = set_competition_name(competition, self.system_dict)
            self.system_dict = set_input_size(input_size, self.system_dict);
            self.system_dict = set_data_root(data_root, self.system_dict);
            self.system_dict = set_batch_size(batch_size, self.system_dict);
            self.system_dict = set_data_shuffle(shuffle_data, self.system_dict);
            self.system_dict = set_num_processors(num_processors, self.system_dict);
            self.system_dict = set_target(target, self.system_dict);
            self.system_dict = set_read(read, self.system_dict);
            self.system_dict = set_percent_data(percent_data, self.system_dict)
            self.system_dict = set_trainval_split(split, self.system_dict)

            if(competition == "bengaliai-cv19"):
                self.system_dict = set_kaggle_dataset_params(self.system_dict);

                self.custom_print("Dataset Details");
                self.custom_print("    Train parquets:     {}".format(self.system_dict["kaggle"]["dataset"]["parquet_files_train"]));
                self.custom_print("    CSV train path:     {}".format(self.system_dict["kaggle"]["dataset"]["csv_train"]));
                self.custom_print("");
                ...
                self.custom_print("    Data Shuffle: {}".format(self.system_dict["dataset"]["params"]["train_shuffle"]));
                self.custom_print("    Processors:   {}".format(self.system_dict["dataset"]["params"]["num_workers"]));
                self.custom_print("    Target:       {}".format(self.system_dict["kaggle"]["dataset"]["target"]));
                self.custom_print("    Percent Data: {}".format(self.system_dict["kaggle"]["dataset"]["percent_data"]));
