...
class finetune_dataset(system):
    ...
    def set_dataset_dataloader(self, test=False):
        if(test):
            if(self.system_dict["kaggle"]["status"]):
                x=0;
            ...
            if(self.system_dict["kaggle"]["status"]):
                if(self.system_dict["kaggle"]["competition"] == "bengaliai-cv19"):
                    sampled_dataset = None;
                    image_datasets = {};
                    dataset_parquet_files = self.system_dict["kaggle"]["dataset"]["parquet_files_train"];
                    csv_train = self.system_dict["kaggle"]["dataset"]["csv_train"];
                    batch_size = self.system_dict["dataset"]["params"]["batch_size"];
                    shuffle = self.system_dict["dataset"]["params"]["train_shuffle"];
                    num_workers = self.system_dict["dataset"]["params"]["num_workers"];
                    target = self.system_dict["kaggle"]["dataset"]["target"];
                    data_root = self.system_dict["kaggle"]["dataset"]["data_root"];
                    read = self.system_dict["kaggle"]["dataset"]["read"];
                    train_val_split = self.system_dict["dataset"]["params"]["train_val_split"];

                    img_list, label_list, self.system_dict["dataset"]["params"]["classes"] = par_csv_bengali_ai(csv_train[0], target) 

                    self.system_dict["dataset"]["params"]["num_classes"] = len(self.system_dict["dataset"]["params"]["classes"]);

                    sampled_dataset = DatasetCustomKaggleBengaliAI(dataset_parquet_files, img_list, 
                        label_list, read).transform_first(self.system_dict["local"]["data_transforms"]["train"]);
